...
def doConfusion(filename):
    ...
    # print("theWorkSpace")
    # print(theWorkSpace)
    #TODO:: comment out the below line. this line make it so that we only go through pills-blue and run-pink workspaces and not the rest. this is because currently we haven't calculated all of the gold totals
    if("pills-blue" in theWorkSpace or "run-pink" in theWorkSpace):
        start_time = delta_t
...
def inner_loop(pfile):
    ...
    if("info.json" in pfile):
        ...
        if filename != "None":
            ...
            sesh_title = (filename["user_id"] + " " +  filename["workspace"]).encode("utf-8")
            final_res[sesh_title] = {"video" : 0, "data" : 0}
            person = filename["user_id"].encode("utf-8")
            workspace = filename["workspace"].encode("utf-8")
